"""Generate the 5-fold ensemble validation predictions."""
...
def generate_validation_ensemble(
    val_df: pd.DataFrame,
    models_dir: Path,
    cfg: dict,
    max_length: int,
    ...
    for fold in range(num_folds):
        ...
        model.eval()
        preds = det_predict(model, tokenizer, val_df[text_col].tolist(), max_length, device=device)
    ...
    out = pd.DataFrame({
        id_col: val_df[id_col].values,
        role_col: cfg["validation_role"],
        "true_stress": val_df[target_col].values.astype(float),
        "prediction": final,
