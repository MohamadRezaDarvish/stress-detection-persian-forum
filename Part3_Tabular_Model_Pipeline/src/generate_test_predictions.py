...
def generate_test_predictions(models: list, prepared: dict, project_config: dict):
    raw = prepared["raw"]["test"]
    mean, std, matrix = ensemble_predict(models, prepared["X"]["test"], project_config["prediction_range"])
    output = pd.DataFrame({
